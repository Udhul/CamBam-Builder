...
- `roughing_clearance` is a signed horizontal allowance exposed for Profile, Pocket
  and Engrave. Zero uses the normal compensated Profile/Pocket boundary or the exact
  Engrave line placement; positive leaves material for a later finishing pass and
  negative overcuts. The exposed CannedCycle Drill keeps zero; CamBam's documented
  roughing-clearance support for drilling applies to spiral drilling, which this MCP
  slice does not author.
...
other compound contours. Corner overcut can intentionally cut beyond concave corners
for insertion clearance but does not remove every cutter-reach limitation. Engrave
...
allowing EndMill or the native `VCutter` enum;

...
The direct output gate reparses all nine events/moves through the bounded
...
inheriting a pass. The accepted native-derived CamBam post has the same nine
semantic items, two cone sweeps and five section-rest values. Byte-for-byte
G-code identity is not required. This is a headless **reference dialect** slice,

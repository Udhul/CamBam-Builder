...
def tool_definitions():
    descriptions = {
        ...
        "document_inspect": "Inspect a revision-consistent paginated inventory, including typed geometry for supported primitives and explicit Profile/Pocket/Engrave/Drill parameters. This read-only call has no request ledger; omit request_id.",
        ...
        "machining_calculate_depth_increment": "Calculate a rounded through-cut depth increment from stock thickness, cut-through allowance and either an exact pass count or a maximum material/tool-safe depth increment. Returns every depth below stock surface and reports whether at least one third of the final pass remains in stock. This read-only advisory call has no request ledger; omit request_id. It is not a material/tool safety calculator or a veto: preserve valid user-requested constraints, surface warnings, and confirm the result before adding a MOP.",
        "machining_configure_part": "Create or update a Part's explicit stock definition and native CamBam nesting settings. Set stock dimensions/material before adding MOPs; choose Grid or IsoGrid with rows, columns and spacing to repeat the entire Part's operations, rather than copying geometry. The operation preserves explicit user choices and returns the resolved settings.",

...
def tool_definitions():
    descriptions = {
        ...
        "document_inspect": "Inspect a revision-consistent paginated inventory, including stable entity IDs, typed geometry for supported primitives and preservation-aware Profile/Pocket/Engrave/Drill parameters. MOP parameters are raw modeled values present in native XML; parameter_metadata reports each field's native Value/Default/omitted state and method applicability, so a Default value is cached text rather than an evaluated CAM-style value. target_group distinguishes a live framework group source from explicit targets, and unsupported_fields names only opaque native paths while unrelated typed parameters remain available. To determine or repeat a manual edit, inspect both a trustworthy baseline and the current snapshot, match stable IDs, and propagate only the measured common delta; absolute placement alone does not prove translation, rotation, alignment or layout intent. Omit expected_revision to discover the current revision after a stale follow-up, then retry a mutation with that revision and a new request_id. A valid request_id is accepted but ignored.",
        ...
        "machining_add_drill": "Add a CannedCycle, SpiralMill_CW or SpiralMill_CCW Drill operation at Points entries or Circle centers. CannedCycle uses peck_distance, retract_height and dwell. SpiralMill uses hole_diameter, signed radial roughing_clearance, drill_lead_out, spiral_flat_base and lead_out_length; its effective cut diameter is hole_diameter - 2*roughing_clearance and must exceed tool_diameter. A nonzero lead_out_length requires drill_lead_out=true; a positive centerward length cannot exceed the effective hole radius. Omit hole_diameter for CamBam Auto only when every target is a Circle; Point targets require an explicit diameter. tool_profile is native cutter metadata; omitted CannedCycle uses Drill and omitted SpiralMill uses Unspecified. Selected relevant values are emitted with state=Value; fresh SpiralMill XML omits CannedCycle-only fields and an unused CustomScript so CamBam does not prompt to reconcile irrelevant defaults. Imported native fields remain preservation-safe. This mutation requires expected_revision set to the current document revision and a fresh request_id. Choose Drill for machining at point or circle-center locations, not for tracing a circle boundary. MOPs append in call order; drill enclosed features before an Outside Profile that cuts their containing part loose. The part name must be project-unique and is created when absent.",

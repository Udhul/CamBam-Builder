...
def tool_definitions():
    descriptions = {
        ...
        "machining_add_engrave": "Add an Engrave operation whose tool center follows selected root Rect/Circle/Arc/Pline/Text curves with no cutter-radius compensation. EndMill remains the default tool profile; choose Vcutter explicitly for V-cutter text or line engraving. This mutation requires expected_revision set to the current document revision and a fresh request_id. Choose it only when centerline tracing or engraving is intended, not as a substitute for dimensionally accurate inside/outside contour cutting. MOPs append in call order; engrave a contained part before cutting that part loose. The part name must be project-unique and is created when absent.",
        ...
        "machining_add_profile": "Add a cutter-radius-compensated Profile around root Rect/Circle/Pline/Region geometry. On closed boundaries, Outside keeps the selected boundary as the finished exterior part edge and Inside keeps it as the finished opening edge. On an open Pline, Inside/Outside selects the offset relative to vertex traversal order; reversing vertex order swaps the physical side, and the result and diagnostic make that directional interpretation explicit. Use Automatic holding tabs when a cutout needs retention; Manual tab placement is not exposed. A tab_distance of zero delegates count to Minimum Tabs. This mutation requires expected_revision set to the current document revision and a fresh request_id. Use Inside for a through-opening when releasing a slug is intended; use Pocket only to clear its whole area. Set corner_overcut=true only when the user wants CamBam to overcut inside corners for a round endmill; it can remove extra material along adjacent sides. MOPs append in call order: create enclosed/detail operations first, and if an Outside Profile cuts their containing part loose, create that cutout last. For through-cuts, choose a material/tool-safe depth increment whose multiples slightly exceed total depth and whose final pass still cuts meaningful stock before crossing the stock bottom; confirm the proposal with the user. The result echoes side semantics and target IDs: verify them against inspected geometry. The part name must be project-unique and is created when absent. Do not silently invent machining parameters.",
        ...
        "machining_configure_part": "Create or patch a Part's explicit stock definition and native CamBam nesting settings. Omitted fields preserve an existing Part; a new minimally specified Part starts with zero/unspecified stock. Stock offset and surface are independent of dimensions. Choose Grid or IsoGrid with rows, columns and spacing to repeat the entire Part's operations, rather than copying geometry. Imported Manual/PointList nesting can be inspected and is preserved by unrelated patches, but this tool does not author their placement data. A Part with no MOPs has nothing to nest, so inspect each Part's MOP membership before export. default_spindle_speed is session-only framework context and does not survive XML export/re-import, so set spindle_speed explicitly on every durable MOP. The operation returns the resolved settings.",

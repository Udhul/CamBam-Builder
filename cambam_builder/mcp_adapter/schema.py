...
def tool_definitions():
    descriptions = {
        ...
        "machining_add_engrave": "Add an Engrave operation whose tool center follows selected root Rect/Circle/Arc/Pline/Text curves with no cutter-radius compensation. EndMill remains the default; VCutter is CamBam's exact native enum token (the UI displays V-Cutter). Both are supported tool-shape selections for this ordinary path-following operation; neither fills Text interiors nor requests skeleton or width/depth-varying V-carving. This mutation requires expected_revision set to the current document revision and a fresh request_id. Choose it only when centerline tracing or engraving is intended, not as a substitute for dimensionally accurate inside/outside contour cutting. MOPs append in call order; engrave a contained part before cutting that part loose. The part name must be project-unique and is created when absent.",
        ...
        "machining_configure_part": "Create or patch a Part's explicit stock definition and native CamBam nesting settings. Omitted fields preserve an existing Part; a new minimally specified Part starts with zero/unspecified stock. stock_offset is local to machining_origin: the returned stock_drawing_origin equals their XY sum with stock_surface as Z. Part stock overrides top-level Machining stock for that Part. Choose Grid or IsoGrid to repeat every MOP in the Part over the same source geometry; do not clone geometry. Nesting does not enlarge Part stock, so every nested outermost toolpath must still fit the configured stock; multi-copy configurations return a diagnostic requiring that check. nest_spacing is clearance between outermost generated toolpaths, not centers or geometry bounds. Imported valid Manual/PointList placement data is preserved by unrelated patches, but this tool does not author it; changing methods removes placement fields that belong to the old method. A Part with no MOPs has nothing to nest. default_spindle_speed is session-only framework context and does not survive XML export/re-import, so set spindle_speed explicitly on every durable MOP. The operation returns resolved and derived stock settings.",

...
def tool_definitions():
    descriptions = {
        ...
        "document_export": "Default delivery for every new or modified client-local document: return the current revision as complete UTF-8 CamBam .cb XML. The client must write content unchanged to its own project workspace and verify sha256 when possible; no server file is created and absolute server paths are unnecessary. A valid request_id is accepted for client compatibility but ignored because this read-only call has no request ledger.",
        "document_import": "Default input for an existing client-local file and the resynchronization step after a user edits that file manually or a server session restarts: import the complete current UTF-8 CamBam .cb XML into a new volatile revision-0 document, then use the returned handle and source sha256. source_name is a leaf name, not a server path; importing never updates an older handle. If that handle also has MCP edits newer than the last export actually written by the client, preserve both versions and ask the user which changes to keep or merge.",
        ...
        "document_inspect": "Inspect a revision-consistent paginated inventory, including typed geometry for supported primitives and explicit Profile/Pocket/Engrave/Drill parameters. Omit expected_revision to discover the current revision after a stale follow-up, then retry a mutation with that revision and a new request_id. A valid request_id is accepted but ignored.",
        "document_list": "Discover this server boot and its currently open volatile documents, including each handle, revision, source hash and summary. Use it after conversational context loss or reconnect; an empty list or changed boot means client-local files must be read and imported again. A valid request_id is accepted but ignored.",
        ...
        "geometry_add_region": "Add one root Region from one closed outer contour plus optional closed hole contours, creating its named layer when absent; identifier and layer must be different project-unique names when the layer is new; XY topology is validated.",
        ...
        "machining_configure_part": "Create or update a Part's explicit stock definition and native CamBam nesting settings. Set stock dimensions/material before adding MOPs; choose Grid or IsoGrid with rows, columns and spacing to repeat the entire Part's operations, rather than copying geometry. default_spindle_speed is session-only framework context and does not survive XML export/re-import, so set spindle_speed explicitly on every durable MOP. The operation preserves explicit user choices and returns the resolved settings.",

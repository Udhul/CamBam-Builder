...
def create_server(service: DocumentService) -> Server:
    ...
    server = Server(
        ...
        instructions=(
            ...
            + " in every tool call. Default to portable content exchange for files in the "
            "client's project workspace. For an existing file, the client reads it and calls "
            "document_import with complete .cb XML. For a new or edited document, call "
            "document_export and have the client write its content unchanged in the client "
            "workspace, optionally checking the returned hash. Never call document_save or "
            "ask for access to its absolute_path for client-local delivery. document_open and "
            "document_save are only for files explicitly requested inside the configured MCP "
            "server workspace. Read-only list, inspect, export and planning calls may omit "
            ...
            "versions. Before a later edit of a client-local file, reread and hash that file. "
            "If it changed since the last import/export, if the server boot changed, or if no "
            "matching handle remains, call document_import with the complete current content "
            "and continue from its new revision-0 handle; never apply a manual file change to "
            ...
            "do not clone geometry when the user asks for part-level nesting. Layer display "

...
def create_server(service: DocumentService) -> Server:
    ...
    server = Server(
        ...
        instructions=(
            ...
            + " in every tool call. First determine whether client and server share a "
            "filesystem. If they do, document_list returns workspace_path: for an existing "
            "or manually edited client-local file, make an exact binary copy under a fresh "
            "workspace-relative .cb name, verify the source and copy hashes, then call "
            "document_open with expected_sha256. Never reconstruct XML through model text, "
            "fall back to an older workspace file, or infer parser incompatibility from a "
            "CONTENT_MISMATCH. The successful open is a new revision-0 snapshot; use that "
            "new handle. When the filesystems differ, read the complete file and use "
            "document_import; its expected_sha256 guards exact transport. For delivery on "
            "a shared filesystem, document_save creates an exact non-overwriting handoff "
            "artifact in the configured MCP workspace: copy its absolute_path with a "
            "deterministic binary operation and verify the destination sha256. The current "
            "handle remains authoritative until that destination changes. Its structured "
            "result says client_file_created=false: never report a client-local task complete "
            "until that copy and verification succeed. Never treat the server path itself as "
            "the client destination or pass an arbitrary client path to document_save. When "
            "the filesystems differ, use document_export inline. Its result explicitly says "
            "delivery=inline_content_only and file_created=false: write its content unchanged "
            "and verify sha256. document_export does not create any file: never claim "
            "delivery until that write succeeds. A hash mismatch means delivery failed; correct "
            "the bytes rather than accepting newline or encoding changes. Read-only list, inspect, export and planning calls may omit "
            "request_id; a valid extra request_id is ignored. Use document_list after "
            "a reconnect or lost conversational context to discover this boot's live handles, "
            "revisions and source hashes. Handles are volatile working snapshots, not file "
            "versions. Copy every handle exactly from a successful result or document_list; "
            "never reconstruct, abbreviate or retype it. Every edit of an existing document requires expected_revision set to "
            "the latest returned revision and a fresh request_id. Never issue mutations in "
            "parallel against the same document: each success increments the revision, so "
            "wait for it and pass its returned revision to the next mutation. Parallel "
            "read-only calls or mutations against different documents are allowed. Before a later edit of a client-local "
            "file, reread and hash that file. "
            "If it changed since the last open/import/export, if the server boot changed, or if no "
            "matching handle remains, use exact staging plus document_open on a shared filesystem, "
            "or document_import only across hosts, and continue from its new revision-0 handle; never apply a manual file change to "
            "an older handle. If that older handle also has MCP edits newer than the last "
            "successfully written export, preserve both versions, export the MCP candidate "
            "under a different client-local name, and ask the user which changes to keep or "
            "merge; never overwrite either side automatically. On STALE_REVISION, call document_inspect without "
            "expected_revision, review the current snapshot, and retry a still-applicable edit "
            "using its current revision and a new request_id. After export, write content "
            "unchanged and verify sha256 before treating the client file as synchronized. "
            "When asked what changed or to repeat a user's edit, compare a baseline and current "
            "snapshot by stable entity IDs. Propagate only the measured delta common to the "
            "demonstrated entities; do not infer alignment, rotation, layout or other intent "
            "from absolute positions. If no trustworthy baseline exists or the delta is "
            "ambiguous, ask before mutating. "
            "Select CAM operations by machining intent: Profile offsets "
            "inside/outside a closed boundary (including Text outlines) or to a "
            "vertex-order-relative side of an open Pline, Pocket clears a closed interior "
            "including Text interiors, and Engrave follows a curve or Text line placement "
            "when roughing clearance is zero (EndMill and VCutter are supported "
            "tool-shape selections, but neither fills Text interiors or turns Engrave into "
            "width/depth-varying V-carving). Signed roughing clearance offsets Profile, "
            "Pocket boundaries and Engrave paths; positive values leave stock and negative "
            "values overcut. Drill uses point or circle "
            "center locations. Do not silently invent target depth, feeds, spindle speed or "
            "stock thickness; when they are not established, elicit them with a reasoned "
            "suggestion only from known stock, material and tool context. For a through-cut, "
            "use machining_calculate_depth_increment rather than doing pass arithmetic by hand: "
            "let D=stock_surface-target_depth and S=stock thickness. After respecting the "
            "tool/material stepdown limit, choose pass count n and depth increment I so n*I "
            "slightly exceeds D, (n-1)*I is less than S, and the final pass retains meaningful "
            "stock engagement (normally at least one third of that pass) before crossing the "
            "stock bottom; ask the user to confirm the proposal. These are advisory heuristics: "
            "honor valid values explicitly requested by the user, warn when they diverge, and "
            "do not reject or silently replace them solely for violating the recommendation. "
            "For computed or multi-point "
            "geometry, compare returned/inspected world coordinates and curved bounds with "
            "the requested dimensions, center, symmetry and containment before adding MOPs "
            "or exporting. Recompute after clarifications rather than reusing prior values. "
            "Every new primitive identifier must be a project-unique name different from its "
            "layer name, because both share the document identifier namespace. "
            "When the user asks to turn existing closed shapes into one Region, use "
            "geometry_replace_with_region with the existing outer and hole UUIDs; do not "
            "recreate guessed coordinates or leave duplicate source contours. The replacement "
            "is atomic, requires all sources on one layer, and reports every removed UUID. "
            "When the user instead asks to change an existing Region's contours, use "
            "geometry_update_region so its UUID, layer and Profile/Pocket targets remain "
            "intact; do not add a duplicate Region. Region holes are excluded islands when "
            "the Region is pocketed. "
            "MOPs are appended within a Part in tool-call order. Plan the complete machining "
            "sequence before adding them: normally machine enclosed/internal features and "
            "non-releasing details first, and put a through-cut Outside Profile that releases "
            "their containing part last. Ask whether an operation cuts through the stock when "
            "its depth or holding intent is unclear. Configure explicit stock and native part "
            "nesting with machining_configure_part: omitted fields preserve an existing Part, "
            "stock offset is local to the Part machining origin (inspection reports their "
            "drawing-space sum), stock surface is the top Z plane, and Grid/IsoGrid repeats "
            "the whole Part's MOPs over shared source geometry. Stock definition and nested "
            "fit checks are optional; when stock is defined, checking the nested envelope is "
            "good practice and any fit diagnostic is advisory only. "
            "Imported valid Manual/PointList placement data is preserved but "
            "is not authored by that tool. Use Automatic Profile holding tabs when retention "
            "is required; manual tab points are not exposed, and tab lead-ins are unavailable "
            "while the exposed Profile lead-in type is None. A Part with no MOPs has nothing "
            "to nest. Do not clone geometry when the user asks for part-level nesting. Layer display "
            "style can be set with document_set_layer_properties and is independent of CAM. "
